...
<div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 16px; margin-bottom: 20px;">
    @if($hasBatch)
        <a href="{{ route('admin.traceability.index', request()->except('status', 'tab', 'page')) }}" style="text-decoration: none;">
        ...
        <a href="{{ route('admin.traceability.index', array_merge(request()->except('status', 'tab', 'page'), ['status' => 'In process'])) }}" style="text-decoration: none;">
        ...
        <a href="{{ route('admin.traceability.index', array_merge(request()->except('status', 'tab', 'page'), ['status' => 'Repaired'])) }}" style="text-decoration: none;">
        ...
        <a href="{{ route('admin.traceability.index', array_merge(request()->except('status', 'tab', 'page'), ['status' => 'BER'])) }}" style="text-decoration: none;">
...
<div class="card">
    <div class="card-header">
        <div>
            ...
            <div class="card-subtitle">Technical diagnostics, replaced component matrix, and unit repair tracking</div>
        </div>
        <div style="display: flex; gap: 10px; flex-wrap: wrap;">
            ...
            <a href="{{ route('admin.traceability.print', request()->query()) }}" target="_blank" class="btn btn-primary">
    ...
    <div style="padding: 14px 20px; background: #F8FAFC; border-bottom: 1px solid var(--dftm-border);">
        <form action="{{ route('admin.traceability.index') }}" method="GET" style="display: flex; gap: 14px; flex-wrap: wrap; align-items: center;">
            @if(request('tab'))
            ...
            <div style="display: flex; gap: 8px; align-self: flex-end;">
                ...
                @if(request()->anyFilled(['outgoing_slip_id', 'batch_id', 'status', 'tab', 'brand']))
                    <a href="{{ route('admin.traceability.index') }}" class="btn btn-outline" style="height: 38px; color: #DC2626; border-color: #FECACA;" title="Reset all filters">
...
<div class="modal-backdrop" id="traceabilityModal">
    <div class="modal-card">
        <form id="traceabilityForm" method="POST" data-base-action="{{ url('admin/traceability/__ID__') }}">
            @csrf
            @method('PUT')
...
<script>
const LOOKUP_URL = "{{ route('admin.traceability.lookup') }}";
const BASE_INDEX_URL = "{{ route('admin.traceability.index') }}";
