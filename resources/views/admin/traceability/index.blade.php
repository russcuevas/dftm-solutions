...
@section('content')
        ...
        <div class="card-body" style="padding: 16px 20px;">
            <div style="display: flex; flex-wrap: wrap; align-items: center; justify-content: space-between; gap: 14px;">
                ...
                <div style="display: flex; align-items: center; gap: 10px;">
                    <a href="{{ route('admin.traceability.createBatch') }}" class="btn btn-accent btn-sm" style="font-weight: 700;">
                        <i class="bi bi-plus-lg"></i> + Create New Batch
                    </a>
                    @if($selectedBatch)
                        <a href="{{ route('admin.traceability.print', ['batch_id' => $selectedBatch->id]) }}" target="_blank" class="btn btn-outline btn-sm">
                            <i class="bi bi-printer"></i> Print Matrix Report
                        </a>
                        <form action="{{ route('admin.traceability.destroyBatch', $selectedBatch->id) }}" method="POST" onsubmit="return confirm('Dissolve this Batch? Units will be returned to the incoming pool.');" style="display: inline;">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="btn btn-outline btn-sm" style="color: #DC2626;" title="Delete Batch">
    ...
    @if($selectedBatch)
        ...
        <!-- Columns: No – Material Diagnostic – Replace Parts – Status - Brand – model -Serial number- Mac Address- Box # - Action -->
        <div class="card">
    ...
    @else
        ...
        <div class="card" style="text-align: center; padding: 60px 20px;">
            ...
            <p style="color: var(--dftm-slate); max-width: 480px; margin: 8px auto 20px auto;">
                Create your first Traceability Batch by selecting incoming scanned units to assign a Batch Number, diagnoses, and repair statuses.
            </p>
            <div>
                <a href="{{ route('admin.traceability.createBatch') }}" class="btn btn-primary">
...
document.addEventListener('DOMContentLoaded', function() {
    const saveItemUrl = '{{ route('admin.traceability.saveItem') }}';
    const bulkUpdateUrl = '{{ route('admin.traceability.bulkUpdate') }}';
