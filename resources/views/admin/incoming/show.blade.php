...
<div class="card">
    <div class="card-header">
        <div>
            <div class="card-title">
                <i class="bi bi-file-earmark-text-fill"></i> Incoming Repair Slip: {{ $batch->slip_no }}
            </div>
            <div class="card-subtitle">Encoded by {{ $batch->encoder->name ?? 'System' }} on {{ $batch->created_at->format('M d, Y h:i A') }}</div>
        </div>
        <div style="display: flex; gap: 10px;">
            <a href="{{ route('admin.incoming.print', $batch->id) }}" target="_blank" class="btn btn-primary btn-sm">
                <i class="bi bi-printer"></i> Print Slip
            </a>
            <a href="{{ route('admin.incoming.edit', $batch->id) }}" class="btn btn-outline btn-sm">
    ...
    <div class="card-body">
        <!-- Excel-like Header Summary Card -->
        <div style="background: #F8FAFC; border: 1px solid var(--dftm-border); border-radius: var(--radius-md); padding: 20px; margin-bottom: 24px;">
            ...
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(220px, 1fr)); gap: 16px;">
                ...
                <div>
                    <span style="font-size: 0.72rem; font-weight: 700; text-transform: uppercase; color: var(--dftm-slate);">BRAND:</span>
                    <div style="font-weight: 600;">{{ $batch->brand ?? 'N/A' }}</div>
                </div>
                <div>
                    <span style="font-size: 0.72rem; font-weight: 700; text-transform: uppercase; color: var(--dftm-slate);">MODEL:</span>
                    <div style="font-weight: 600;">{{ $batch->model ?? 'N/A' }}</div>
                </div>
            </div>
        </div>

        <!-- Serialized Units Subtable -->
        <h3 style="font-size: 1rem; font-weight: 700; color: var(--dftm-navy); margin-bottom: 12px; display: flex; align-items: center; gap: 8px;">
            <i class="bi bi-cpu"></i> Incoming Serialized Units ({{ $batch->items->count() }} items)
        </h3>
