    ...
    <div class="card-header">
        <div>
            <div class="card-title">
                <i class="bi bi-file-earmark-check-fill"></i> Outgoing Repair Slip: {{ $slip->slip_no }}
            </div>
            <div class="card-subtitle">Processed by {{ $slip->encoder->name ?? 'System' }} on {{ $slip->created_at->format('M d, Y h:i A') }}</div>
        </div>
        <div style="display: flex; gap: 10px;">
            <a href="{{ route('admin.outgoing.edit', $slip->id) }}" class="btn btn-accent btn-sm">
                <i class="bi bi-pencil-square"></i> Edit Outgoing Slip
            </a>
            <a href="{{ route('admin.outgoing.print', $slip->id) }}" target="_blank" class="btn btn-primary btn-sm">
                <i class="bi bi-printer"></i> Print Outgoing Slip
            </a>
            <a href="{{ route('admin.outgoing.index') }}" class="btn btn-outline btn-sm">
                <i class="bi bi-arrow-left"></i> Back to Outgoing Slips
            </a>
        </div>
    </div>

    <div class="card-body">
        <!-- Excel-like Header Summary Card for Outgoing Slip -->
        <div style="background: #F8FAFC; border: 1px solid var(--dftm-border); border-radius: var(--radius-md); padding: 20px; margin-bottom: 24px;">
            ...
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(180px, 1fr)); gap: 16px;">
                ...
                <div>
                    <span style="font-size: 0.72rem; font-weight: 700; text-transform: uppercase; color: var(--dftm-slate);">BATCH & BOX:</span>
                    <div style="font-weight: 600;">{{ $slip->batch_no }} @if($slip->box_no) - {{ $slip->box_no }} @endif</div>
                </div>
                <div>
